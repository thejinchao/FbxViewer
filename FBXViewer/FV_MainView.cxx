...
#define PLAY_ANIMATION              200
	...
	bool InitializeOpenGL()
	{
		...
		glClearColor(0.28f, 0.28f, 0.28f, 0.0f);
	...
	void FV_MainView_Enter(void)
	{
		...
		gSceneContext = new SceneContext(DEFAULT_WINDOW_WIDTH, DEFAULT_WINDOW_HEIGHT, lSupportVBO);
	...
	{
		if (gSceneContext->GetSceneID() != timerID) return;
		...
		glutTimerFunc((unsigned int)gSceneContext->GetFrameTime().GetMilliSeconds(), TimerCallback, timerID);
	}
	...
	void DisplayCallback()
	{
		...
		if (gSceneContext->GetStatus() == SceneContext::MUST_BE_LOADED)
		{
			...
			gSceneContext->LoadScene();

			CreateMenus();

			// Call the timer to display the first frame.
			glutTimerFunc((unsigned int)gSceneContext->GetFrameTime().GetMilliSeconds(), TimerCallback, gSceneContext->GetSceneID());
		}
	}
	...
	{
		gSceneContext->LoadFile(szFileName);
		glutPopWindow();
		glutPostRedisplay();
	}
