...
#include "FV_SceneCache.h"
#include "FV_SetCamera.h"
#include "FV_DrawScene.h"
...
namespace FbxViewer
{
	...
	SceneContext::SceneContext(int pWindowWidth, int pWindowHeight, bool pSupportVBO)
		: mFileName(NULL), mSceneID(0), mStatus(UNLOADED),
	...
	void SceneContext::ClearScene()
	{
		...
		{
			//delete light cache
			int lLightCount = mScene->GetSrcObjectCount<FbxLight>();
			for (int lLightIndex = 0; lLightIndex < lLightCount; ++lLightIndex)
			{
				FbxLight * lLight = mScene->GetSrcObject<FbxLight>(lLightIndex);
				LightCache * lLightCache = static_cast<LightCache *>(lLight->GetUserDataPtr());
				if (lLightCache) delete lLightCache;
				lLight->SetUserDataPtr(0);
			}
	...
	bool SceneContext::LoadFile(const char * pFileName)
	{
	...
	void SceneContext::OnMouseMotion(int pX, int pY)
	{
		...
		default:
			break;
		case CAMERA_ORBIT:
			if (mStatus == UNLOADED || mStatus == MUST_BE_LOADED) break;
